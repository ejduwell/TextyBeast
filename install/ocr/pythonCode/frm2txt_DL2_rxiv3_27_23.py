...
def frm2txt_mmocr_det(img_dir, tag, out_dir, detector,recognizer,x_merge,det_ckpt_in,recog_ckpt_in):
    ...
    # %% Parameters
    out_dir = img_dir+'/'+out_dir
    #mmocr = MMOCR(det=detector, recog=recognizer) #orig..(ejd 3/6/23)
    #mmocr = MMOCR(det=detector, recog=recognizer,det_ckpt='/scratch/g/tark/dataScraping/envs/ocr/env/mmocrChkpts/panet_r18_fpem_ffm_sbn_600e_icdar2015_20210219-42dbe46a.pth',recog_ckpt='/scratch/g/tark/dataScraping/envs/ocr/env/mmocrChkpts/sar_r31_parallel_decoder_academic-dba3a4a3.pth') #new..(ejd 3/6/23)
    ...
    for ii in range(0, n_im):
        ...
        if len(contour_list) > 1:
            ...
            for b_boxes in xy_MaxMin:
                ...
                yrng = list(range(b_boxes[3], b_boxes[2])) #orig
                #yrng = list(range(b_boxes[3], b_boxes[2]-1)) # ejd attempted fix for IndexError: index 1080 is out of bounds for axis 0 with size 1080 3/22/23
            ...
            for ww in ydim_gaps_b:
                ycut = ((ydim_gaps_b[itr8r] + ydim_gaps_e[itr8r]/2)) #orig
                
                ysiz = (ydim_gaps_e[itr8r]-ydim_gaps_b[itr8r])
                #ycut = (((ydim_gaps_b[itr8r] + ydim_gaps_e[itr8r])-1)/2) #EJD ADDED -1 to fix error 3/22/23
            ...
            if main_lsts is None:
                ...
                for zz in main_lsts:
                    ...
                    if itr8r >= len(main_lsts):
                        ...
                        if countr == 0:
                            try: tmp_lst
                            except NameError: tmp_lst = None
                            if tmp_lst is None:
                                #tmp_lst = copy.deepcopy(zz)
                                tmp_lst = [[]] #EJD ADDED 3/22/23
                                tmp_lst = np.zeros((2, len(tmp_lst)), dtype=np.uint8)
