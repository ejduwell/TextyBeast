...
def fill_hole(input_mask):
    ...
    #canvas = canvas[1:h + 1, 1:w + 1].astype(np.bool) # orig
    canvas = canvas[1:h + 1, 1:w + 1].astype(np.bool_) #EJD.. added underscore..
...
def centralize(points_yx,
    ...
    #step_flags = np.ones((len(points_yx), 1), dtype=np.bool) # orig
    step_flags = np.ones((len(points_yx), 1), dtype=np.bool_) # EJD update..
